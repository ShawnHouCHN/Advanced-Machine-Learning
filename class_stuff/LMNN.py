...
class LargeMarginNearestNeighbor(KNeighborsClassifier):
    ...
    def fit(self, X, y):
        ...
        L = minimize(fun=self.loss_simple, x0=self.L_init, method=self.method,
        ...
        super(LargeMarginNearestNeighbor, self).fit(self.transform(X), y)
                ...
                if j < self.k:
                    ...
                    if np.size(impostors) > 0:
                        for imp in impostors:
                            p1 = 2 * np.dot((X_transformed[i,].T - X_transformed[index_j,].T),
                                            np.reshape((self.X[i,] - self.X[index_j,]), (1, self.d)))
                            p2 = 2 * np.dot((X_transformed[i,].T - X_transformed[imp,].T),
                                            np.reshape((self.X[i,] - self.X[imp,]), (1, self.d)))
                            jac += (1 - omega_1) * (p1 - p2)
                        jac += omega_1 * 2 * np.dot((X_transformed[i,].T - X_transformed[index_j,].T),
                                                    np.reshape((self.X[i,] - self.X[index_j,]), (1, self.d)))
...
###################################################################################################################
    ...
    def loss_simple(self, L_in):
        ...
        # calculate pairwise distances between all samples in transformed X
        distance_matrix = euclidean_distances(self.transform(self.X), squared=True)
        ...
        for i in range(self.m):
            for j in range(self.k):
                ...
                if np.size(impostors) > 0:
                    ...
                    jac += self.omega[0] * 2 * np.dot((X_transformed[i, ].T - X_transformed[index_j, ].T), np.reshape((self.X[i, ] - self.X[index_j, ]), (1, self.d)))
                # if there is NO impostors for given pair x_i and x_j
                else:
                    jac += self.omega[0] * 2 * np.dot((X_transformed[i, ].T - X_transformed[index_j, ].T), np.reshape((self.X[i, ] - self.X[index_j, ]), (1, self.d)))
